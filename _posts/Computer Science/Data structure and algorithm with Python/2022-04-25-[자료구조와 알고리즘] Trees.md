...
Binary tree는 모든 각 노드가 두 자식을 최대로 갖는 구조이다.(자식 노드가 최대 2개)

A Binary tree is one in which each node has **a maximum of two children**. The nodes in the binary tree are organized in the form of left sub-tree and right sub-tree.
...
This mode of traversal is implemented <mark>using a queue data structure</mark>. Starting with the root node, we push it into a queue. The node at the front of the queue is accessed (dequeued) and either printed or stored for later use.
...
A binary search tree is a tree that is structurally a binary tree, and stores data in its nodes very efficiently. 
...
- 두 개로 만드는 경우
  ...
  - ```python
    ...
    t.inorder()
    ```
  - inorder 함수를 호출할 때 인자를 넣어주지 않아도 되기 때문에 번거롭게 느껴지지 않고 헷갈리지 않는다.
