...
- Scalability – process can take advantage of multiprocessor architectures Utilization of MP Architectures 
...
- The library supports for thread creation, scheduling in user space with no kernel support  Kernel is unaware of user-level threads, so fast 
  - Drawbacks 
    - If the kernel is single threaded, then any user-level thread performing a blocking system call will cause the entire process to block, even if other threads are available to run within the applicatio
...
- Kernel-supported Threads Supports for threads is provided by OS 
  ...
  -  Generally slower to manage threads than user thread 
  - But, if a thread performs a blocking system call, the kernel can schedule another thread for execution 
...
- Most PL do not enable programmers to specify concurrent activities except Ada 
...
![image-20220926172027205](https://raw.githubusercontent.com/speardragon/save-image-repo/main/img/image-20220926172027205.png)

![image-20220926172036706](https://raw.githubusercontent.com/speardragon/save-image-repo/main/img/image-20220926172036706.png)
