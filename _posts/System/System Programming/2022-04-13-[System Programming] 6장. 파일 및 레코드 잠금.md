...
이 두 프로세스 간 indepenent하게 동작하도록 한다.



그러나 정보 교환이 가능하게 해야 할 경우도 있을 것이다.
...
- 한 프로세스가 그 영역을 읽거나 수정할 때 다른 프로세스의 접근 을 제한
...
(3) 프로세스 B가 잔액에 입금액을 더하여 레코드를 수정핚다: 잔액 120만원 

(4) 프로세스A가 잔액에 입금액을 더하여 레코드를 수정핚다: 잔액 110만원
...
- cmd 
  - F_LOCK : 지정된 영역에 대해 잠금을 설정한다. 이미 잠금이 설정되어 있으면 잠금이 해제될 때까지 기다린다. 
  - F_TLOCK : 지정된 영역에 대해 잠금을 설정한다. 이미 잠금이 설정되어 있으면 기다리지 않고 오류(-1)를 반환한다. 
...
```shell
$ chmod 2644 mandatory.txt (set-group-ID 비트 설정) 
$ ls -l mandatory.txt 
...
```shell
$ file_lock mandatory.txt  #강제 잠금 설정이 되어 있음
 - 
```
...
- fcntl() 시스템 호출을 이용하여 지정된 영역에 대해 잠금 검사, 잠금 설정 혹은 잠금 해제를 핛 수 있다. 
