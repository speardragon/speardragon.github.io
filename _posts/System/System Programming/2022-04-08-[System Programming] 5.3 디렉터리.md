...
- write
  ...
  - Directory: directory의 file을 생성, 삭제할 수 있는가? 
    ...
    - Write() 시스템콜을 이용해 디렉터리 엔트리 수정, 추가하는 것 불허
...
root directory의 i-노드를 찾아간다. -> 2

2번 노드를 찾아가면 그것이 루트 디렉토리의 i-노드 이다.

- i-노드에는 UID, GID, Type, 권한, 블록 포인터 등이 들어있고 그 중에서 블록 포인터를 들어가면 그곳에 directory entry(file name + i-node)가 존재한다.
...
struct dirent *readdir(DIR *dp);
 ...
 //디렉토리의 current file position은 읽을 때마다 읽은 디렉터리 엔트리 (구조체
 //dirent) 크기 맊큼 이동한다
...
main (int argc, char ** argv)
{
    ...
    if (argc == 1) { //실행 파일 이름만 주어진 경우
    	...
    	strcpy(pathname, argv[1]); //인자를 변수에 저장
    }
...
int main(int argc, char **argv)
{
    ...
    if (argc == 1)
    	dir = "."; //current directory로 대상으로 지정
...
struct passwd{ 			/* pwd.h */
    char *pw_name; 		/* Username. 이것을 가져오는 것임*/
...
그런 다음 4번 노드를 찾아가면 usr의 i-노드 정보가 있는데 블록포인터를 보고 202번 블락 번호의 내용으로 들어가 test.c의 노드 번호인 6번을 또 타고 들어가 204번 블럭을 찾아 들어간다.
